...
Sistema de registro automático de incidentes para mesa de ayuda mediana: tres canales de entrada (correo, web, teléfono) convergen vía N8N en una API FastAPI que clasifica cada incidente con un pipeline híbrido (reglas determinísticas ≥0,90 → Gemini 2.5 Flash → fallback) hacia los cinco sectores canónicos (Seguridad Informatica, Soporte Tecnico Hardware, Soporte Tecnico Software, Bases de Datos, Sistemas), persiste en PostgreSQL con trazabilidad completa y deriva los casos de confianza <0,70 a revisión humana. Los resultados del corpus sintético (92 % exactitud, F1 macro 0,919, −89 % de tiempo de registro, 9,5 % de intervención humana) fueron descartados con el corpus y quedan superados por C-27; se re-miden sobre el corpus real JSON multietiqueta.
