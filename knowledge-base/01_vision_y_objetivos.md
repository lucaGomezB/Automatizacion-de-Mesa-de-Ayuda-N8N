...
| Sector responsable (Seguridad Informatica / Soporte Tecnico Hardware / Soporte Tecnico Software / Bases de Datos / Sistemas) | Recibir tickets bien derivados | Corregir clasificaciones erróneas (validación → corpus) |
| Organización | Liberar ~200 horas-persona/trimestre | Trazabilidad completa; cumplimiento Ley 25.326 |
| Equipo de investigación | Validar empíricamente la hipótesis | Corpus etiquetado multietiqueta (JSON); métricas reproducibles |
...
- Clasificación automática híbrida en 2 etapas (reglas determinísticas → Gemini 2.5 Flash) hacia 5 sectores: **Seguridad Informatica**, **Soporte Tecnico Hardware**, **Soporte Tecnico Software**, **Bases de Datos**, **Sistemas** (strings exactos, sin tildes; `Operaciones` eliminado).
...
- Evaluación experimental sobre corpus JSON multietiqueta (`sector_asignado` + `sectores_adicionales`) con métricas estándar.
...
> **Nota (C-27):** los valores reportados de exactitud, F1, tiempo e intervención humana provienen del corpus sintético de 200 casos, hoy **descartado y eliminado**. Quedan **superados**; se re-medirán sobre el corpus real JSON multietiqueta.

| Métrica | Objetivo (tesis) | Resultado reportado (superado — corpus sintético) |
|---|---|---|
| Exactitud global de clasificación | ≥ 85 % | **92 %** (IC 95 % Wilson [87,2 ; 95,2]) — a re-medir |
| F1 macro promediado | ≥ 0,85 | **0,919** — a re-medir |
| Tiempo medio de registro | Reducción significativa | 165,3 s → **18,2 s** (−89 %, Wilcoxon p < 0,001) — a re-medir |
| Intervención humana | Reducción sustancial | 100 % → **9,5 %** — a re-medir |
| Resolución en etapa determinística | Maximizar (costo/latencia) | **~62 %** de los casos sin invocar Gemini — a re-medir |
