...
### DD-08 — asyncpg en lugar de psycopg2 (decision confirmada)
**Decisión**: El código usa `asyncpg` + SQLAlchemy async. **Justificación**: la decisión se fundamenta en la necesidad de E/S no bloqueante coherente con el event loop de FastAPI/uvicorn. La tesis ha sido actualizada para reflejar esta elección.
...
### DD-10 — Clasificación dentro de POST /incidentes (decision confirmada)
**Decisión**: No existe `POST /api/v1/clasificar` independiente; la clasificación es parte de la creación del incidente. La tesis ha sido actualizada para reflejar la arquitectura real.
...
### DD-12 — JWT sobre SSO para desarrollo
**Decisión**: Se implementó autenticación mediante JWT (python-jose + bcrypt) con tabla `users`. **Justificación**: La autenticación única corporativa (SSO) se difiere a entornos productivos, donde requeriría un proveedor de identidad externo (Azure AD, Keycloak) e integración OIDC/SAML. La arquitectura actual permite migrar a SSO sin cambios estructurales: el endpoint de login se reemplazaría por un flujo de redirección OIDC, y el resto de la cadena de validación de tokens permanece igual.
