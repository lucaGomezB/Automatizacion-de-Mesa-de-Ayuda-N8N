...
\texttt{directorio\_empleado} & id (PK), legajo (UNIQUE), \\ email (UNIQUE), telefono, \\ sector\_id (FK), rol, activo, \\ user\_id (FK) & Directorio institucional de empleados para resolución de contacto y notificación. Datos de contacto almacenados en texto plano para su auditabilidad directa; el campo \texttt{rol} distingue usuario final, operador y administrador del directorio. \\
\bottomrule
\end{tabular}
\end{table}

La entidad central \texttt{incidente} referencia mediante claves foraneas a los catalogos de sector, estado y canal de origen, y mantiene una relacion uno a muchos con \texttt{clasificacion\_log} para preservar la trazabilidad histórica de las decisiones del clasificador. El número de incidente que se comunica al usuario corresponde al identificador único del registro, formateado para su legibilidad; la incorporación de un prefijo configurable se prevé como evolución futura (Capítulo~10). El esquema completó, incluyendo restricciones de integridad referencial e indices secundarios, se documenta en el Anexo~C. El directorio de empleados (\texttt{directorio\_empleado}) se vincula opcionalmente con la tabla de autenticación mediante \texttt{user\_id} y referencia el catálogo de sectores mediante \texttt{sector\_id}; su campo \texttt{rol} distingue al usuario final, al operador y al administrador del directorio.
