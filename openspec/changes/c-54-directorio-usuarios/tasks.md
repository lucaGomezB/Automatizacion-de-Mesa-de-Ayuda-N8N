...
## 9. Fix NIT-1: alcance VIS-001 en los endpoints de clasificaciones

- [x] 9.1 RED: escribir `App/Backend/tests/test_clasificacion_visibility.py` que exija que un no administrador (sector A) reciba 404 en `GET /api/v1/clasificaciones/incidente/{id}` y en `PATCH /api/v1/clasificaciones/{log_id}/validar` para un incidente de sector B, que el `PATCH` NO modifique la fila, que el administrador suceda, que una cuenta con alcance vacio reciba 404 y que el anonimo reciba 401. Verificacion: `pytest tests/test_clasificacion_visibility.py -m "not integration"` falla en los casos fuera de alcance/vacio y pasa en admin/in-sector/anonimo (4 failed, 5 passed).
- [x] 9.2 GREEN: extender `ClasificacionService.list_by_incidente`/`validate`/`validate_payload` con `alcance` (verificacion de `permite_sector` ANTES de mutar) y propagar `AlcanceDep` (`get_alcance_incidentes`, reutilizado de incidentes) en las rutas de clasificaciones. Verificacion: los 9 tests de 9.1 en verde.
- [x] 9.3 TRIANGULATE: cubrir lectura/validacion dentro de alcance (allow), fuera de alcance (deny), administrador, alcance vacio, acceso anonimo, y que la mutacion se aplica en alcance y NO se aplica fuera de alcance (re-lectura directa de la fila). Verificacion: `pytest tests/test_clasificacion_visibility.py -q` en verde (9).
- [x] 9.4 REFACTOR: reutilizar el servicio/dependencia de visibilidad existente (`incident_visibility`/`get_alcance_incidentes`) sin duplicar la derivacion rol -> sector; sin cambios a clasificacion, cinco sectores ni cifrado Fernet. Verificacion: sin codigo de visibilidad duplicado; `ruff check .` limpio.
- [x] 9.5 (docs/spec): actualizar el spec `incident-visibility` (VIS-001, escenarios de clasificaciones), agregar `design.md` D16 y estos tasks. Verificacion: `openspec validate --strict --changes c-54-directorio-usuarios` sin errores.
- [x] 9.6 Verificacion final del fix: `pytest -m "not integration"`, `pytest -m integration`, `ruff check .`, `pytest tests/test_openapi_sync.py -v` y regenerar `docs/openapi.json` si cambio. Verificacion: todo en verde, sin regresiones.