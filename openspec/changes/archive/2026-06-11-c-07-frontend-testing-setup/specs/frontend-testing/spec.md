...
El proyecto SHALL proveer una infraestructura de pruebas automatizadas para el frontend React ejecutable por comando. La configuración SHALL usar Vitest como runner, un entorno DOM (`happy-dom`) que permita renderizar componentes React, el alias `@/` resuelto al directorio `src/` (espejando `vite.config.ts`), y un archivo de setup que registre los matchers de `@testing-library/jest-dom` para todas las pruebas. El proyecto MUST exponer un script `test` en `Frontend/package.json` que ejecute la suite, y un medio para generar el reporte de cobertura. Las dependencias de prueba MUST agregarse solo como `devDependencies`, sin alterar las dependencias de runtime.

#### Scenario: La suite se ejecuta por comando

- **WHEN** se ejecuta el script `test` definido en `Frontend/package.json`
